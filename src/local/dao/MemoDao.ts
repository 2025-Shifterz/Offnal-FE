...
export class MemoDao {
  ...
  async getAllMemos(): Promise<Todo[]> {
    ...
    try {
      const [result] = await db.executeSql('SELECT * FROM todos WHERE type = ?;', ['memo']);
      const memos: Todo[] = [];
  ...
  async createMemo(memoText: string, targetDate?: dayjs.Dayjs): Promise<Todo> {
    ...
    try {
      ...
      const [insertedRowResult] = await db.executeSql('SELECT * FROM todos WHERE id = ?', [newId]);
   ...
   */
  async updateMemo(id: number, updates: { text?: string; completed?: boolean }): Promise<boolean> {
    const db = await openShifterzDB();
    try {
      ...
      const query = `UPDATE todos SET ${setParts.join(', ')} WHERE id = ? AND type = ?;`;
      params.push(id, 'memo');
  ...
  async deleteAllMemos(): Promise<void> {
    const db = await openShifterzDB();
    try {
      const [result] = await db.executeSql('DELETE FROM todos WHERE type = ?;', ['memo']);
      const success = (result?.rowsAffected || 0) >= 0;
