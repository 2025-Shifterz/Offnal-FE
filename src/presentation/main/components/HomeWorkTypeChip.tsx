...
const HomeWorkTypeChip: React.FC<HomeWorkTypeChipProps> = ({dayType, workType}) => {
    ...
    const chipDescriptionClassName = clsx(
        "text-heading-xxxs pt-number-3",
        {
    ...
    const chipTitle = dayType === DayType.PAST ? "어제 근무" :
        dayType === DayType.TODAY ? "오늘 근무" : "내일 근무";
    ...
    return (
        <View className={containerClassName}>
            ...
            <View className="flex-col items-start justify-center ps-number-8">
                <Text className="text-heading-xxxs font-medium text-text-subtle-inverse">
                    {chipTitle}
                </Text>
                <Text className= {chipDescriptionClassName}>
                    {workType}
                </Text>
            </View>
        </View>
