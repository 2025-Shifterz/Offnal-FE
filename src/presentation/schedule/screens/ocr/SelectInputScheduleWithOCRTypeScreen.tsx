...
const { WorkSheetModule } = NativeModules;
...
const SelectInputScheduleWithOCRTypeScreen = () => {
  ...
  const hadleOCRResponse = async (response: ImagePickerResponse) => {
    ...
    try {
      ...
      const resultJsonString = await ScheduleModule.parseFromBase64(asset.base64);


      console.log(resultJsonString);


      if (resultJsonString.error) {
        Alert.alert('OCR Error', resultJsonString.error);
      } else {
        console.log('OCR Result:', resultJsonString);
      }
