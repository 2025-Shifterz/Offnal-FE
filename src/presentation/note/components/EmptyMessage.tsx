...
const EmptyMessage = ({ text, divStyle, textStyle, iconSize }: EmptyTodoContentProps) => {
  return (
    <View className={twMerge('items-center gap-[19px]', divStyle)}>
      ...
      <View>
        <Text
          style={{ color: '#B1B8BE' }}
          className={twMerge('text-center body-s', textStyle)}
        >{`아직 등록된  ${text}${text === '할 일' ? '이' : '가'} 없어요.`}</Text>
        <Text style={{ color: '#B1B8BE' }} className={twMerge('text-center body-s', textStyle)}>
          {text === '할 일'
